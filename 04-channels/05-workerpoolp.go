...
// this syntax restricts the direction of each channel.
// For THIS specific function, we will only SEND to completedJobsChan and RECEIVE from jobsChan
func worker(id int, jobsChan <-chan int, completedJobsChan chan<- int) {

	for j := range jobsChan { // iterates (and RECEIVES) each and all the jobs in the channel.
